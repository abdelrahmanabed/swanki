...
const [sortOrder, setSortOrder] = useState("new");
 ...
 useEffect(() => {
  ...
  const fetchItems = async () => {
    ...
    const filteredItems = response.data.filter(item => item.typeofI === "bags");
  ...
  useEffect(() => { 
    ...
    async function fetchItems() {
      try {
        ...
        let sortedItems = response.data.filter(item => item.typeofI === "bags");;
  ...
  return (
    ...
    <div className={`flex-col p-0.5 pr-0 space-y-2  filters ${hidden? "absolute flex opacity-100 z-50 w-44": 'hidden'} md:relative md:flex ml-2 mr-2 md:m-0 md:mr-2 pt-2   w-32 md:w-48 lg:w-52 `}>
       ...
       <label className="flex space-x-2 cursor-pointer  items-center "><input type="checkbox" className="" name="model" value="bb" onChange={handleCheck} checked={checkedFilters.model.includes("bb")}></input> <span>Backpack</span></label>
        <label className="flex space-x-2 cursor-pointer  items-center "><input type="checkbox" className="" name="model" value="hb"onChange={handleCheck} checked={checkedFilters.model.includes("hb")}></input> <span>Handbag</span></label>
        <label className="flex space-x-2 cursor-pointer  items-center "><input type="checkbox" className="" name="model" value="cb"onChange={handleCheck} checked={checkedFilters.model.includes("cb")}></input> <span>Crossbag</span></label>
